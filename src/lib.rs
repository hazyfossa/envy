use std::{collections::HashMap, env as sys, error::Error, ffi::OsString, path::PathBuf};
...
pub trait EnvironmentParse<Repr>: Sized {
    ...
    fn env_deserialize(raw: Repr) -> Result<Self, EnvironmentParseError>;
}
...
impl<T: EnvironmentParse<String>> EnvironmentParse<OsString> for T {
    ...
    fn env_deserialize(raw: OsString) -> Result<Self, EnvironmentParseError> {
        let value = raw
            .into_string()
...
macro_rules! env_parse_raw {
    ($t:ident => $ty:ty) => {
        impl EnvironmentParse<$ty> for $t {
            ...
            fn env_deserialize(raw: $ty) -> Result<Self, $crate::EnvironmentParseError> {
                Ok(Self::from(raw))
            }
        }
    };
...
pub trait Env: EnvRaw + EnvDiff {
    fn get<T: EnvironmentVariable>(&self) -> Result<T, EnvironmentError> {
